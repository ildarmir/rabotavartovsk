class MainpageController < ApplicationController
 ...
 def index
    @vac = @city.vacancies.limit(6).order("created_at DESC")
    pop = @city.vacancies.select("id,position,view,organisation").order("created_at desc").limit(30)
    pop=pop.each {|k| k.view=0 if k.view.nil? }
    pop.sort_by! {|k| k.view}
    @pop=pop.reverse.slice(0..5)
